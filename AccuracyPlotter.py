...
class AccuracyPlotter:
    def __init__(self, learn_rate: float, epochs: int, layers: [int], batch_size: int):
        ...
        self.xLabel = f"Epochs (with Batch Size of {self.batch_size})"
    ...
    def setTestAccuracy(self, accuracy: float):
        self.test_accuracy = [accuracy for _ in range(len(self.epochNumbers))]
    ...
    def showPlot(self):
        ...
        plt.title(
            f"Layers: {self.layers}, LR: {self.learn_rate}, Epochs: {self.epochs}, Training Accuracy: {self.training_accuracy[-1] * 100:.2f}%, Test Accuracy: {self.test_accuracy[-1] * 100:.2f}%")
